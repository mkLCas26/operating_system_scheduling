...
class PrioPage(tk.Frame):
    ...
    def setup_ui(self):
        ...
        tk.Label(input_frame, text="Priority (Low#=High):", **lbl_style).grid(row=2, column=0, padx=(10, 5), pady=2, sticky="e")
        self.entry_prio = tk.Entry(input_frame, **entry_style)
        self.entry_prio.grid(row=2, column=1, padx=5, pady=2, sticky="w")

        # ROW 3: MODE TOGGLE (Preemptive vs Non-Preemptive)
        tk.Label(input_frame, text="Mode:", **lbl_style).grid(row=3, column=0, padx=(10, 5), pady=(2, 10), sticky="e")
        self.mode_var = tk.StringVar(value="Non-Preemptive")
        
        mode_frame = tk.Frame(input_frame, bg="#b5c4ba")
        mode_frame.grid(row=3, column=1, sticky="w", padx=2, pady=(2, 10))
        ...
        btn_frame.grid(row=4, column=0, columnspan=2, pady=(5, 10), padx=10, sticky="ew")
        ...
        tk.Button(btn_frame, text="Start PRIO", command=self.run_algorithm, **btn_style).grid(row=0, column=1, padx=4, sticky="ew")
        ...
        cols = ("PID", "AT", "BT", "PRIO", "CT", "TAT", "WT")
        self.tree = ttk.Treeview(table_frame, columns=cols, show="headings")
        for col in cols:
            self.tree.heading(col, text=col)
            self.tree.column(col, width=35, anchor="center") 
    ...
    def add_process(self):
        ...
        prio = self.entry_prio.get() # Grab the new priority value

        if not at.isdigit() or not bt.isdigit() or not prio.isdigit():
        ...
        self.process_queue.append({'pid': pid, 'at': int(at), 'bt': int(bt), 'prio': int(prio)})
        self.tree.insert("", "end", values=(pid, at, bt, prio, "-", "-", "-"))
    ...
    def run_algorithm(self):
        ...
        # Check our Retro Radiobutton to see if we are in Preemptive Mode
        is_preemptive = (self.mode_var.get() == "Preemptive")

        calculated_data, avg_tat, avg_wt, cpu_util, timeline = calculate_priority(self.process_queue, is_preemptive)

        for item in self.tree.get_children():
            self.tree.delete(item) 
            
        for p in calculated_data:
            self.tree.insert("", "end", values=(p['pid'], p['at'], p['bt'], p['prio'], p['ct'], p['tat'], p['wt']))
