class MFTBestFit:
    @staticmethod
    def calculate_allocation(total_mem_limit, part_sizes, process_sizes, strategy="best"):
        ...
        diary_report = "📝 --- CHRONICLE ALLOCATION DIARY (BEST-FIT STRATEGY) ---\n\n"
        ...
        for job_id, job_size in process_sizes.items():
            ...
            # Find partition with least internal fragmentation (best fit)
            for p in partitions:
                if p["type"] == "standard" and p["assigned_job"] is None and p["size"] >= job_size:
                    internal_frag = p["size"] - job_size
                    if internal_frag < best_frag:
                        best_frag = internal_frag
                        best_partition = p
            
            if best_partition is not None:
                best_partition["assigned_job"] = job_id
                best_partition["job_size"] = job_size
                best_partition["internal_frag"] = best_frag
                
                total_internal_frag += best_frag
                total_active_used_ram += job_size
                allocated_jobs_list.append(job_id)
                
                diary_report += f"🟢 [ALLOCATED] -> {job_id} ({job_size}k) resides inside {best_partition['id']}. (Internal Frag: {best_frag}k)\n"
                allocated = True
