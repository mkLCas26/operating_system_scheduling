...
try:
    from mft_worst_fit_process import MFTWorstFit
except Exception:
    from .mft_worst_fit_process import MFTWorstFit

class TransparentMFTWorstFitSimulator(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("MFT: Worst-Fit")
    ...
    def setup_layout(self):
        ...
        self.lbl_title = tk.Label(header, text="MFT MEMORY ALLOCATION & ANALYSIS ENGINE (WORST-FIT STRATEGY)", 
        ...
        self.btn_calc = tk.Button(btn_pane, text="▶ Allocate", font=("Comic Sans MS", 10, "bold"), bg="#22c55e", fg="#ffffff", bd=0, width=12, pady=5, command=self.run_worst_fit_allocation)
    ...
    def reset_to_defaults(self):
        ...
        self.run_worst_fit_allocation()

    def run_worst_fit_allocation(self):
        try:
            ...
            messagebox.showerror("Validation Error", "Error: Make sure all fields contain valid positive integers!")
            return

        try:
            result = MFTWorstFit.calculate_allocation(total_mem_limit, part_sizes, process_sizes)
...
if __name__ == "__main__":
    app = TransparentMFTWorstFitSimulator()
    app.mainloop()
